...
class EditorCamera:
    ...
    def update(self):
        ...
        if Mouse.getButton(Mouse.buttons.MOUSE_BUTTON_MIDDLE):
            ...
            Singletons.app.scene.camera.position -=  worldRel * self.dragSpeed*(1/Singletons.app.scene.camera.zoom)
        Singletons.app.scene.camera.zoom += Mouse.scrollDelta.y*Time.dt*math.exp(Singletons.app.scene.camera.zoom)*self.zoomSpeed
